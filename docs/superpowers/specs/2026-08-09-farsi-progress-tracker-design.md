...
- **Verb conjugation helpers.** `conjugatePresent(presentStem)` → the six present forms (`می‌` + stem + ‑م/‑ی/‑د/‑یم/‑ید/‑ند). Glide ی is inserted for vowel-final stems: always for ا-final stems (آ → می‌آیم), and for و-final stems only when the و is a vowel — a small known-stem set (گو → می‌گویم، جو). Consonantal-و stems like رو (rav‑) take no glide: می‌روم. `conjugatePast(pastStem)` → six past forms (stem + ‑م/‑ی/‑∅/‑یم/‑ید/‑ند). Used by flashcards and stem review cards.
...
**Auth for claude.ai connectors: OAuth 2.1 (MCP auth spec).** claude.ai's connector UI does not support static headers, so the app is also a minimal OAuth provider:

- Discovery: `/.well-known/oauth-protected-resource` (RFC 9728, points at this app as the authorization server) and `/.well-known/oauth-authorization-server` (RFC 8414, lists endpoints; PKCE S256 required; `token_endpoint_auth_methods_supported: ["none"]`). `/api/mcp` 401 responses carry `WWW-Authenticate: Bearer resource_metadata="…/.well-known/oauth-protected-resource"` so clients discover the flow.
- `POST /oauth/register` (RFC 7591 dynamic client registration, open): stores `{client_name, redirect_uris[]}` in `oauth_clients`, returns a public `client_id` (no secret).
- `GET /oauth/authorize` (session-gated — middleware sends anonymous users to `/login?next=…`, which round-trips back after sign-in): validates client_id + exact redirect_uri match + PKCE S256 challenge, renders a consent page, and on approve (plain `POST /oauth/authorize/approve` form route for testability) issues a single-use 10-minute code (`oauth_codes`, sha256-stored) and redirects with `code` + `state`.
- `POST /oauth/token` (public): `authorization_code` grant only — validates code (unused, unexpired, client + redirect_uri match) and PKCE verifier, then mints a standard `fpt_` API token named after the client (`api_tokens` — same table, so `/api/mcp`'s `authenticateToken` is unchanged and connector tokens are listed/revocable in `/settings`). Returns `{access_token, token_type: "bearer"}` — non-expiring, no refresh tokens; revocation is deletion in `/settings`.
- Tables `oauth_clients` / `oauth_codes` are server-managed only: RLS enabled with NO policies for browser roles; grants to service_role.
- `/login` gains a validated `?next=` redirect (internal paths only) so the authorize round-trip works.
