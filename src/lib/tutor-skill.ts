...
Rules: exercise ids unique; "typed" with "input":"script" exercises production (hardest — use sparingly early on); set "vocab_id" whenever the exercise tests a tracked vocab item so the SRS learns from it (use the item's real id from get_due_vocab or search_vocab; omit the field otherwise); distractor options should be plausible (same word class or theme); every "typed" exercise needs ONE short, unambiguous answer — state the expected format in prompt.text (e.g. "Type one word in English") and list accepted variants in "expected"; never ask an open question with several possible answers in a single typed blank (use choice or match for those).
