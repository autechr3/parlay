...
import { sendEmail, unsubscribeUrl, EMAIL_HEAD } from "../_shared/email.ts";
...
Deno.serve(async (req) => {
  ...
  for (const u of users ?? []) {
    ...
    const errorsHtml = (topErrors ?? [])
      .map((e: { error: string; occurrences: number }) => `<li>${e.error} (${e.occurrences}×)</li>`).join("");
    const nextHtml = (nextLessons ?? [])
      .map((l: { number: number; title: string; slug: string }) =>
        `<li>L${l.number} <a href="${site}/lessons/${l.slug}">${l.title}</a></li>`).join("");
    ...
    try {
      ...
      console.error(`send failed for ${u.id}: ${e}`); // claim row stays — no retry storm; next week sends again
    }
  }
